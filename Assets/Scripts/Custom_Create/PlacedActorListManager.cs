...
public class PlacedActorListManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        else if (!string.IsNullOrEmpty(selectedModulePath) && File.Exists(selectedModulePath))
        {
            ...
            // FIX: Clear NewModuleName so subsequent loads don't accidentally
            // hit the new-module branch (e.g. after QR import goes to StartScreen
            // and then user taps the module again)
            ...
            generateQRButton.onClick.AddListener(() =>
            {
                ...
                if (!string.IsNullOrEmpty(savedPath))
                {
                    ...
                    generateQRButton.GetComponentInChildren<Text>().text = "View Generated QR";
                    // FIX: Show popup immediately — no need for a second tap
                    ShowQRPopup();
                }
            });
        }
        ...
        else if (!string.IsNullOrEmpty(selectedModulePath) && File.Exists(selectedModulePath))
        {
            ...
            generateQRButton.onClick.AddListener(() =>
            {
                ...
                if (!string.IsNullOrEmpty(savedPath))
                {
                    ...
                    generatedQRPath = savedPath;
                    generateQRButton.GetComponentInChildren<Text>().text = "View Generated QR";
                    // Show popup immediately — no need for a second tap
                    ShowQRPopup();
                }
            });
        }
